...
//        $this->middleware('auth');
        ...
        foreach ($cateProject as $cate) {
            $htmlCateProject .= '<li class=" col-xs-12 col-sm-6">';
            $htmlCateProject .= '<div class="itemMenu">';
            $htmlCateProject .= '<a class="linkTo" data-title="Dự án thiết kế" href="' . route('cateListProject', [$cate->id, $cate->alias]) . '"></a>';
            $htmlCateProject .= '<img src="http://viphomes.vn/image/cache/catalog/tin tuc/thiet-ke-noi-that-chung-cu-146x109.jpg" alt="">';
            $htmlCateProject .= '<span>' . $cate->name . '</span>';
            $htmlCateProject .= '</div>';
            $htmlCateProject .= '</li>';
        }
        //TODO group cate project
        ...
        foreach ($cateDesign as $cate) {
            ...
            $htmlDesign .= '<img src="http://viphomes.vn/image/cache/catalog/tin tuc/thiet-ke-noi-that-chung-cu-146x109.jpg" alt="">';
            $htmlDesign .= '<span>' . $cate->name . '</span>';
            $htmlDesign .= '</div>';
            $htmlDesign .= '</li>';
        }

        view()->share(['thisCateProject' => $htmlCateProject, 'thisDesign' => $htmlDesign]);
    }
    ...
    public function index()
    {
        ...
        foreach ($getSlider as $get) {
            if (isset($get)) {
                ...
                $htmlSlider .= '<a href="' .$get->addSlug.'" title="' . $get->images . '">';
                $htmlSlider .= '<img class="responsive" src="' . asset('upload/slider/' . $get->images) . '" alt="' . $get->images . '"/>';
                $htmlSlider .= '</a>';
                $htmlSlider .= '</div>';
            }
        }
        ...
        //TODO partner
        $getPartner = DB::table('partners')->get();
        foreach ($getPartner as $get) {
            $htmlPartner .= '<div><img src="' . asset('upload/partner/' . $get->images) . '"/></div>';
        }
        //TODO get Feng Shui
        ...
        foreach ($getNewsFeng as $get) {
            ...
            $htmlNewNews .= '<a  alt="' . $get->title . '" href="#">' . $get->title . '</a>';
            $htmlNewNews .= '</h4>';
            $htmlNewNews .= '<div class="blog_item_motangan">' . truncateStringWords($get->summary, 150) . '</div>';
            $htmlNewNews .= '<a alt="' . $get->title . '" href="#" class="blog_itemt_link">Xem thêm</a>';
            $htmlNewNews .= '</div>';
            $htmlNewNews .= '</div>';
            $htmlNewNews .= '</div>';
        }
        ...
        return view('flexshop.index')->with(['thisFeatureProject' => $htmlFeatureProject, 'thisFeatureProduct' => $htmlFeatureProduct, 'thisFeatureThietke' => $htmlFeatureThietke, 'thisFeatureNoithat' => $htmlFeatureNoithat, 'thisSlider' => $htmlSlider, 'thisLoaiSp' => $htmlLoaiSp, 'thisNews' => $htmlNews, 'thisPartner' => $htmlPartner, 'thisFengshui' => $htmlFengshui, 'thisNewNews' => $htmlNewNews,'thisChungcu'=>$htmlChungcu,'thisNhapho'=>$htmlNhapho]);
    }
...
//            ->where('id', $getCateId)
        ...
        $htmlBreadcrumb .= '</ul>';
        $htmlBreadcrumb .= '</div>';
        return view('flexshop.singleProduct')->with(['data' => $getData, 'thisRelate' => $htmlRelate, 'thisBreadcrumb' => $htmlBreadcrumb]);
    }

    //TODO here Design project **************************************
    public function getListDesign()
    {
        ...
        return view('flexshop.design')->with(['thisListProject' => $htmlContent, 'thisCateHere' => $htmlCate, 'thisHot' => $htmlHot]);
    }
    public function cateDesign($id)
    {
        ...
        return view('flexshop.design')->with(['thisListProject' => $htmlContent, 'thisBreadcrumb' => $htmlBreadcrumb, 'thisCateHere' => $htmlCate, 'thisHot' => $htmlHot]);
    }
    public function singleDesign($id)
    {
        ...
        $htmlContent .= '</div>';
        //TODO get Category
        $getCate = DB::table('cate_furs')->select('id', 'name', 'alias')->get();
        foreach ($getCate as $cate) {
            if (!empty($cate)) {
                $htmlCate .= '<li class="cat-item"><a href="' . route('cateListDesign', [$cate->id, $cate->alias]) . '" data-name="' . $cate->name . '" data-title="' . $cate->name . '">' . $cate->name . '</a></li>';
            }
        ...
        $htmlBreadcrumb .= '</ul>';
        $htmlBreadcrumb .= '</div>';
        return view('flexshop.singleDesign')->with(['thisPost' => $htmlContent, 'thisCateHere' => $htmlCate, 'thisHot' => $htmlHot, 'thisBreadcrumb' => $htmlBreadcrumb]);
    }
...
//            ->where('chil.lvl', '<>', 0)
        ...
        return view('flexshop.fengshui')->with(['thisListProject' => $htmlContent, 'thisBreadcrumb' => $htmlBreadcrumb, 'thisCateHere' => $htmlCate, 'thisHot' => $htmlHot]);
    }
    public function singleNews($id)
    {
        ...
        $htmlBreadcrumb .= '</ul>';
        $htmlBreadcrumb .= '</div>';
        return view('flexshop.singleFengshui')->with(['thisPost' => $htmlContent, 'thisCateHere' => $htmlCate, 'thisHot' => $htmlHot, 'thisBreadcrumb' => $htmlBreadcrumb]);
    }
